...
function PublicationEntry({ pub, index }) {
  return (
    <li key={index} className="list-disc w-5/6 mt-2">
      ...
      <span className="">
        {pub.pub_date + " " + pub.pub_month + " " + pub.pub_year !== "" ? pub.pub_year + ". " : ""}
      </span>
      <span className="">
  ...
  return (
    <div>
      {topic === "Middleware and Platforms" || topic === "" ? (
        <div className="mb-2">
          <h2 className="text-4xl font-bold text-primary">Middleware and Platforms</h2>
          <h3 className="text-xl my-2 text-accent">Streaming Processing Middleware</h3>
          <ul>
      ...
      {topic === "Software and System Architecture" || topic === "" ? (
        <div className="mb-2">
          <h2 className="text-4xl font-bold text-primary">Software and System Architecture</h2>
          <h3 className="text-xl my-2 text-accent">Service Oriented Computing</h3>
          <ul>
            {entries["Software and System Architecture"]["Service Oriented Computing"].map(
              (pub, index) => PublicationEntry({ pub, index })
            )}
          </ul>
          <h3 className="text-xl my-2 text-accent">Domain-Specific System Design</h3>
          <ul>
            {entries["Software and System Architecture"]["Domain-Specific System Design"].map(
              (pub, index) => PublicationEntry({ pub, index })
            )}
          </ul>
        </div>
      ...
      {topic === "Modeling and Analysis" || topic === "" ? (
        <div className="mb-2">
          <h2 className="text-4xl font-bold text-primary">Modeling and Analysis</h2>
          <h3 className="text-xl my-2 text-accent">Simulation Method and Tool</h3>
          <ul>
            {entries["Modeling and Analysis"]["Simulation Method and Tool"].map((pub, index) =>
              PublicationEntry({ pub, index })
            )}
          </ul>
        </div>
      ) : (
